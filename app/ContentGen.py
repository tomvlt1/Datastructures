#This file was used for the generation of artificial data so that it woudld be possibele to have a functioning application.
#The in this file we procide set of possible attributes that each user may have, and based on this the program selects a random combination of such attributes and generates a database. 
#The number of users generated by the data is customizabel, be it is currently set to generate 5000 users.

import pandas as pd
import random
from datetime import date

#This part of the file generates the random data, it includes the attributes that we want the projects in the file to contain. This cunction takes in a number of projects the user wants to generate and does so.
...
def generate_random_data(num_rows):
    ...
    data = []
     ...
     #This is where the file generates the data by rndomly selecting a value for each one of the attributes and does so for as many times as the user selected 
...
#Here we save the data to a CSV and we do so by providing the desired file name. 
...
def main():
    ...
    generated_data = generate_random_data(5000) #Here is where we specify the number of users that we want the program to generate as fake data. 
