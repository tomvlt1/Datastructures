...
initial_data = data[data['Type'] == 'Alumni']

mentors_bp = Blueprint('mentors', __name__)

 
@mentors_bp.route('/', methods=['GET', 'POST']) # method POST is when the user inputs something and get the first time the user enters
...
def mentors():
    ...
    try:
        ...
        print(f"Error en la ruta de mentores: {e}")
